    ...
    stages{
       ...
       stage("push helm charts to nexus helm repo"){
        ...
        stage("deploying helm charts to K8cluster"){
            steps{
                script{
                    withCredentials([file(credentialsId: 'k8cred', variable: 'kubernetes')]) {
                       dir('kubernetes/') {
                        sh 'helm upgrade --install --set image.repository="34.122.89.158:8083/saurav" --set image.tag="${VERSION}" --kubeconfig /root/.kube/kconfig javaapp myapp/ --debug'
                      }
                   } 
                }
            }
